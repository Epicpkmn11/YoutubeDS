...
extern unsigned short *fileBrowseBgPal;
...
std::u16string newline = UTF8toUTF16("»");
...
void initGraphics(void) {
	...
	BG_PALETTE[0xff] = 0;
	BG_PALETTE[0xfe] = 0xFBDE;
	BG_PALETTE[0xfd] = 0xBDEF;

	tonccpy(BG_PALETTE, fileBrowseBgPal, 32);

	tonccpy(BG_PALETTE_SUB, BG_PALETTE, 512);
}

void loadFont(void) {
	FILE* font = fopen("/font.nftr", "rb");
...
// ImageData loadBmp16(std::string path, const unsigned char *imageBuffer) {
...
// ImageData loadBmp16(std::string path, const unsigned char *imageBuffer, std::vector<u16> &palette) {
// 	imageBuffer.clear();
// 	palette.clear();
// 	FILE* file = fopen(path.c_str(), "rb");
// 	ImageData imageData = {0, 0};
...
// 		fread(palTemp, 4, 16, file);
// 		for(int i=0;i<16;i++) {
// 			palette.push_back(((palTemp[i]>>27)&31) | ((palTemp[i]>>19)&31)<<5 | ((palTemp[i]>>11)&31)<<10 | 1<<15);
// 		}
...
// 			for(unsigned x=0;x<imageData.width;x+=2) {
// 				u8 val = *(src++);
// 				imageBuffer.push_back(val>>4); // First nibble
// 				imageBuffer.push_back(val&0xF); // Second nibble
// 			}
// 		}
// 	}
// 	fclose(file);
...
// ImageData loadBmp(std::string path, const unsigned char *imageBuffer) {
// 	FILE* file = fopen(path.c_str(), "rb");
...
// ImageData loadPng(std::string path, const unsigned char *imageBuffer) {
...
void drawImage(int x, int y, int w, int h, const unsigned char *imageBuffer, bool top) {
	tonccpy((top ? BG_GFX : BG_GFX_SUB), imageBuffer, (w*h));
	// x /= 2;
	// for(int i=0;i<h;i++) {
	// 	for(int j=0;j<w;j+=2) {
	// 		if(imageBuffer[(i*w)+j] != 0) { // Do not render transparent pixel
	// 			(top ? BG_GFX : BG_GFX_SUB)[(y+i)*128+(j/2)+x] = imageBuffer[(i*w)+j+1] << 8 | imageBuffer[(i*w)+j];
	// 		}
	// 	}
	// }
}

void drawImageFromSheet(int x, int y, int w, int h, const unsigned char *imageBuffer, int imageWidth, int xOffset, int yOffset, bool top) {
	for(int i=0;i<h;i++) {
		for(int j=0;j<w;j+=2) {
			if(imageBuffer[((i+yOffset)*imageWidth)+j+xOffset]>>15 != 0) { // Do not render transparent pixel
				// (top ? BG_GFX : BG_GFX_SUB)[((y+i)*128)+j+x] = imageBuffer[((i+yOffset)*imageWidth)+j+xOffset];
			}
		}
	}
}

void drawImageFromSheetScaled(int x, int y, int w, int h, double scaleX, double scaleY, const unsigned char *imageBuffer, int imageWidth, int xOffset, int yOffset, bool top) {
	if(scaleX == 1 && scaleY == 1)	drawImageFromSheet(x, y, w, h, imageBuffer, imageWidth, xOffset, yOffset, top);
		...
		drawImageScaled(x, y, w, h, scaleX, scaleY, buffer.data(), top);
	}
}

void drawImageScaled(int x, int y, int w, int h, double scaleX, double scaleY, const unsigned char *imageBuffer, bool top) {
	if(scaleX == 1 && scaleY == 1)	drawImage(x, y, w, h, imageBuffer, top);
	else {
		for(int i=0;i<(h*scaleY);i++) {
			for(int j=0;j<(w*scaleX);j++) {
				if(imageBuffer[(int)((((int)(i/scaleY))*w)+(j/scaleX))]>>15 != 0) { // Do not render transparent pixel
					(top ? BG_GFX : BG_GFX_SUB)[(y+i)*128+x+j] = imageBuffer[(int)((((int)(i/scaleY))*w)+(j/scaleX))];
				}
			}
		}
	}
...
void drawImageTinted(int x, int y, int w, int h, u16 color, const unsigned char *imageBuffer, bool top) {
	for(int i=0;i<h;i++) {
		for(int j=0;j<w;j++) {
			if(imageBuffer[(i*w)+j]>>15 != 0) { // Do not render transparent pixel
				// (top ? BG_GFX : BG_GFX_SUB)[(y+i)*128+j+x] = color & imageBuffer[(i*w)+j];
			}
		}
	}
}
...
void printText(std::string text, int xPos, int yPos, bool top, bool invert) { printTextTinted(UTF8toUTF16(text), WHITE, xPos, yPos, top, invert); }
void printText(std::u16string text, int xPos, int yPos, bool top, bool invert) { printTextTinted(text, WHITE, xPos, yPos, top, invert); }
void printTextCentered(std::string text, int xOffset, int yPos, bool top, bool invert) { printTextTinted(UTF8toUTF16(text), WHITE, ((256-getTextWidth(UTF8toUTF16(text)))/2)+xOffset, yPos, top, invert); }
...
void printTextCenteredTinted(std::string text, u16 color, int xOffset, int yPos, bool top, bool invert) { printTextTinted(UTF8toUTF16(text), color, ((256-getTextWidth(UTF8toUTF16(text)))/2)+xOffset, yPos, top, invert); }
...
void printTextTinted(std::string text, u16 color, int xPos, int yPos, bool top, bool invert) { printTextTinted(UTF8toUTF16(text), color, xPos, yPos, top, invert); }
...
void printTextTinted(std::u16string text, u16 color, int xPos, int yPos, bool top, bool invert) {
	...
	for(unsigned c=0;c<text.size();c++) {
		...
		for(int i=0, j=0;i<tileSize;i++) {
			image[j++] = (fontTiles[i+(t*tileSize)]>>6 & 3);
			image[j++] = (fontTiles[i+(t*tileSize)]>>4 & 3);
			image[j++] = (fontTiles[i+(t*tileSize)]>>2 & 3);
			image[j++] = (fontTiles[i+(t*tileSize)] & 3);
		}
...
void printTextAbsW(std::string text, double w, double scaleY, int xPos, int yPos, bool top, bool invert) { printTextTintedScaled(UTF8toUTF16(text), w/getTextWidth(text), scaleY, WHITE, xPos, yPos, top, invert); }
...
void printTextMaxW(std::string text, double w, double scaleY, int xPos, int yPos, bool top, bool invert) { printTextTintedScaled(UTF8toUTF16(text), std::min(1.0, w/getTextWidth(text)), scaleY, WHITE, xPos, yPos, top, invert); }
...
void printTextScaled(std::string text, double scaleX, double scaleY, int xPos, int yPos, bool top, bool invert) { printTextTintedScaled(UTF8toUTF16(text), scaleX, scaleY, WHITE, xPos, yPos, top, invert); }
...
void printTextCenteredAbsW(std::string text, double w, double scaleY, int xOffset, int yPos, bool top, bool invert) { printTextTintedScaled(UTF8toUTF16(text), w/getTextWidth(text), scaleY, WHITE, ((256-w)/2)+xOffset, yPos, top, invert); }
...
void printTextCenteredMaxW(std::string text, double w, double scaleY, int xOffset, int yPos, bool top, bool invert) { printTextTintedScaled(UTF8toUTF16(text), std::min(1.0, w/getTextWidth(text)), scaleY, WHITE, ((256-getTextWidthMaxW(text, w))/2)+xOffset, yPos, top, invert); }
...
void printTextCenteredScaled(std::string text, double scaleX, double scaleY, int xOffset, int yPos, bool top, bool invert) { printTextTintedScaled(UTF8toUTF16(text), scaleX, scaleY, WHITE, ((256-getTextWidthScaled(text, scaleX))/2)+xOffset, yPos, top, invert); }
...
void printTextCenteredTintedAbsW(std::string text, double w, double scaleY, u16 color, int xOffset, int yPos, bool top, bool invert) { printTextTintedScaled(UTF8toUTF16(text), w/getTextWidth(text), scaleY, color, ((256-w)/2)+xOffset, yPos, top, invert); }
...
void printTextCenteredTintedMaxW(std::string text, double w, double scaleY, u16 color, int xOffset, int yPos, bool top, bool invert) { printTextTintedScaled(UTF8toUTF16(text), std::min(1.0, w/getTextWidth(text)), scaleY, color, ((256-getTextWidthMaxW(text, w))/2)+xOffset, yPos, top, invert); }
...
void printTextCenteredTintedScaled(std::string text, double scaleX, double scaleY, u16 color, int xOffset, int yPos, bool top, bool invert) { printTextTintedScaled(UTF8toUTF16(text), scaleX, scaleY, color, ((256-getTextWidth(text))/2)+xOffset, yPos, top, invert); }
...
void printTextTintedAbsW(std::string text, double w, double scaleY, u16 color, int xPos, int yPos, bool top, bool invert) { printTextTintedScaled(UTF8toUTF16(text), w/getTextWidth(text), scaleY, color, xPos, yPos, top, invert); }
...
void printTextTintedMaxW(std::string text, double w, double scaleY, u16 color, int xPos, int yPos, bool top, bool invert) { printTextTintedScaled(UTF8toUTF16(text), std::min(1.0, w/getTextWidth(text)), scaleY, color, xPos, yPos, top, invert); }
...
void printTextTintedScaled(std::string text, double scaleX, double scaleY, u16 color, int xPos, int yPos, bool top, bool invert) { printTextTintedScaled(UTF8toUTF16(text), scaleX, scaleY, color, xPos, yPos, top, invert); }
...
int getTextWidthScaled(std::string text, double scale) { return getTextWidth(UTF8toUTF16(text))*scale; }
